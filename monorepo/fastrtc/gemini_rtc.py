...
def find_free_port(start_port: int = 7860, max_port: int = 7870) -> int:
    ...
    Returns:
        A free port number or raises OSError if no ports are available
    """
    for port in range(start_port, max_port + 1):
        try:
            ...
            continue
    raise OSError(f"No free ports available in range {start_port}-{max_port}")


if __name__ == "__main__":
    try:
        # Use port 7861 instead of 7860
        port = 7861
        
        if (mode := os.getenv("MODE")) == "UI":
            stream.ui.launch(server_port=port)
        elif mode == "PHONE":
        ...
        print("Please try closing other applications that might be using ports in the range 7860-7870")
        exit(1)
